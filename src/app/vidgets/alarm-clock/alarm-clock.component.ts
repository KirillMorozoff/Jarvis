...
import {DefaultSignalService} from './signals/default/defaultSignal.service';

@Component({
  selector: 'wfm-alarm-clock',
...
export class AlarmClockComponent implements OnInit {
  ...
  ngOnInit() {
    this.speakService.speak('Да сэр... Укажите во сколько включить сигнал?');
  }
  ...
  submit() {
    ...
    let timer = 86400000 - currentTime + alarmTime;
    if (timer > 86400000) { timer = timer - 86400000; }

    const that = this;
    function func() {
      console.log( 'Таймер работает' );
      that.defaultSignalService.speach();
    }

    this.timerId = setTimeout(func, timer);
